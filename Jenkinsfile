        ...
        stage('Verify Credentials') {
            steps {
                script {
                    ...
                    echo '🔧 Validating credential functionality...'
                    script {
                        try {
                            // Test MongoDB connection (if Node.js is available)
                            def mongoTestResult = sh(
                                script: '''
                                    if command -v node >/dev/null 2>&1; then
                                        timeout 10 node -e "
                                            const { MongoClient } = require('mongodb');
                                            MongoClient.connect('${MONGODB_URI}', { serverSelectionTimeoutMS: 5000 })
                                                .then(() => { console.log('✅ MongoDB connection: SUCCESS'); process.exit(0); })
                                                .catch(err => { console.log('❌ MongoDB connection: FAILED -', err.message); process.exit(1); });
                                        " 2>/dev/null || echo "⚠️  MongoDB connection test skipped (Node.js not available)"
                                    else
                                        echo "⚠️  MongoDB connection test skipped (Node.js not available)"
                                    fi
                                ''',
                                returnStatus: true
                            )
                        } catch (Exception e) {
                            echo "⚠️  MongoDB connection test failed: ${e.getMessage()}"
                        }
                    }
