...
export default function Experience({ experiences = [] }: ExperienceProps) {
  ...
  const updateExperience = (index: number, field: keyof IWorkExperience, value: string | string[]) => {
            ...
            <div className="space-y-6">
              {editData.map((exp, index) => (
                <div key={index} className="bg-gray-800 p-6 rounded-xl border border-gray-700">
                  ...
                  <div>
                    <label className="block text-sm text-gray-400 mb-2">Job Details (comma-separated)</label>
                    <input
                      type="text"
                      value={exp.details.join(', ')}
                      onChange={(e) => updateExperience(index, 'details', e.target.value.split(',').map(s => s.trim()).filter(Boolean))}
